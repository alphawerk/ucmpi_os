...
sudo npm install --silent -g --unsafe-perm node-red@^3.x.x > /dev/null 2>&1 || error_exit "Unable to install node-red"
...
npm install epoll mqtt serialport@9.2.8 mitt xml2js bcrypt express express-ws express-handlebars@3.0.0 express-handlebars-layouts express-session memorystore body-parser cookie-parser request express-fileupload xml2js fs-extra path uid-safe https rpi-gpio os child_process > /dev/null 2>&1|| error_exit "Error installing dependancies"
